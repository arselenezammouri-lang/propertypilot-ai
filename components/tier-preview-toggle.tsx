...
const TIER_CONFIG = {
  real: { label: "Vista Reale", icon: Eye, color: "bg-muted/50", textColor: "text-foreground" },
...
export function TierPreviewToggle({ currentRealTier, onTierChange }: TierPreviewToggleProps) {
  ...
  if (!isVisible) {
    return (
      <Button
        ...
        className="fixed top-20 right-4 z-50 bg-background/50 border border-border hover:bg-purple-500/20 text-foreground/60 hover:text-foreground"
      >
  ...
  return (
    <div className="fixed top-20 right-4 z-50">
      <div className="bg-background/90 backdrop-blur-xl border border-border rounded-xl p-4 shadow-2xl w-72">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-xs text-foreground/60">Simulatore Vista</p>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className={`w-full justify-between ${currentConfig.color} border-border ${currentConfig.textColor} hover:bg-muted/50`}
            >
              <div className="flex items-center gap-2">
          ...
          <DropdownMenuContent className="w-56 bg-background/95 border-border">
            <DropdownMenuLabel className="text-foreground/60">Simula Vista Piano</DropdownMenuLabel>
            ...
            {(Object.keys(TIER_CONFIG) as PreviewTier[]).map((tier) => {
              ...
              return (
                ...
                >
                  ...
                  {previewTier === tier && (
                    <Badge className="ml-auto bg-purple-500 text-foreground text-xs">Attivo</Badge>
                  )}
                </DropdownMenuItem>
              );
            })}
        ...
        {tierFeatures && (
          <div className="mt-4 space-y-3">
            <div className="flex items-center justify-between text-xs">
              <span className="text-foreground/60">Limite Annunci:</span>
              <Badge variant="outline" className="border-border text-purple-300">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {tierFeatures.unlocked.slice(0, 4).map((feature) => (
                    <Badge key={feature} className="bg-green-500/20 text-green-300 text-[10px] border-border">
                      {feature}
                    </Badge>
                  ))}
                </div>
