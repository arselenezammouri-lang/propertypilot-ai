      ...
      {isOpen ? <DialogContent className="sm:max-w-lg bg-gradient-to-br from-background via-background to-blue-600/5 border-blue-600/20">
        <DialogHeader className="text-center pb-2">
          ...
          <DialogTitle className="text-2xl font-bold">
            <span className="text-gradient-blue">{step.title}</span>
          </DialogTitle>
        ...
        <div className="py-4">
          <ul className="space-y-3">
            {step.details.map((detail, i) => (
              <li key={i} className="flex items-start gap-3 p-3 rounded-lg bg-muted/30 border border-border/10">
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-3 pt-2">
          <div className="flex gap-2 w-full">
            ...
            <Button 
              onClick={nextStep}
              className={`flex-1 bg-gradient-to-r ${step.gradient} hover:opacity-90 text-foreground`}
            >
              {isLastStep ? (
                <>
