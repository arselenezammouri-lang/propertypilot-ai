  ...
  return (
    <div className="relative">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${planInfo.color} flex items-center justify-center shadow-lg`}>
              {viewingPlan === "free" ? <Sparkles className="h-6 w-6 text-foreground" /> :
               viewingPlan === "starter" ? <Zap className="h-6 w-6 text-foreground" /> :
               viewingPlan === "pro" ? <Crown className="h-6 w-6 text-foreground" /> :
      ...
      {viewingPlan === currentPlan && (
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r ${planInfo.color}/20 border border-border mb-6`}>
              ...
              <CardContent className="p-6">
                ...
                {!locked ? (
                  ...
                  <Link href="/dashboard/billing">
                    <Button 
                      variant="outline" 
                      className="w-full border-border text-foreground/30 hover:border-border"
                      size="sm"
                      disabled
                    >
        ...
        <div className="mt-12">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {lockedFeatures.slice(0, 6).map((feature) => {
              ...
              return (
                <Card 
                  key={feature.id}
                  className="border border-border/60 bg-muted/20 opacity-70"
                >
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-white/[0.05] flex items-center justify-center">
                        <div className="text-foreground/30">{feature.icon}</div>
                      </div>
                      <div className="flex-1">
                        <h4 className="text-sm font-medium text-foreground/40">{featureCopy.name}</h4>
                        <p className="text-xs text-foreground/25 mt-1">{featureCopy.description}</p>
                      </div>
                      <Lock className="h-4 w-4 text-foreground/25" />
                    </div>
                  </CardContent>
                </Card>
              );
          ...
          {nextPlan && (
            <div className="mt-6 text-center">
              <Link href="/dashboard/billing">
                <Button 
                  className={`bg-gradient-to-r ${planInfo.color} hover:opacity-90 text-foreground`}
                  size="lg"
                >
