...
export default function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  function renderBlock(block: string, i: number) {
    ...
    // Paragraph with possible bold text
      ...
      <article className="pt-28 pb-20">
        <div className="max-w-3xl mx-auto px-5 sm:px-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className="text-xs font-medium text-primary bg-primary/10 px-2.5 py-1 rounded-full">{category}</span>
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-8 leading-tight">{title}</h1>
          ...
          <div className="mt-16 pp-card p-8 text-center bg-muted/30">
            <h3 className="text-xl font-bold mb-2">{isIt ? "Pronto a provare?" : "Ready to try it?"}</h3>
            <p className="text-muted-foreground mb-5">{isIt ? "Genera il tuo primo annuncio AI in meno di 30 secondi." : "Generate your first AI listing in under 30 seconds."}</p>
            <Link href="/auth/signup">
          ...
          <div className="mt-12">
            <h3 className="text-lg font-semibold mb-4">{isIt ? "Altri articoli" : "More articles"}</h3>
            <div className="space-y-3">
              {BLOG_POSTS.filter((p) => p.slug !== slug).slice(0, 2).map((p) => (
                <Link key={p.slug} href={"/blog/" + p.slug} className="block pp-card p-4 hover-lift group">
                  <h4 className="font-medium group-hover:text-primary transition-colors">{isIt ? p.titleIt : p.title}</h4>
                  <p className="text-sm text-muted-foreground mt-1">{isIt ? p.excerptIt : p.excerpt}</p>
                </Link>
              ))}
            </div>
          </div>
