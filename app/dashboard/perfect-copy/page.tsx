  ...
  function renderVariantCard(variant: CopyVariant, name: string, icon: React.ReactNode, color: string) {
    ...
    <div className="space-y-4" data-testid={`variant-${name}`}>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <Card className="shadow-lg group bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-muted/20 dark:to-cyan-900/20">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base flex items-center gap-2">
            ...
            {result && (
              <div className="space-y-6">
                ...
                <div className="mt-6 flex items-center gap-3">
                    ...
                    onClick={async () => {
                      try {
                        const res = await fetch('/api/listings/save', {
                          ...
                          credentials: 'include',
                          body: JSON.stringify({
                            title: result.professionale?.titolo || result.emotivo?.titolo || 'Untitled',
                            ...
                            generated_content: {
                              ...
                              titles: [
                                ...
                                result.breve?.titolo,
                                result.seo?.titolo,
                                result.luxury?.titolo,
                              ].filter(Boolean),
                            },
                          }),
                        });
                        if (res.ok) {
                          toast({ title: isItalian ? '✅ Salvato!' : '✅ Saved!', description: isItalian ? 'Annuncio salvato nella tua libreria.' : 'Listing saved to your library.' });
                        } else {
                          throw new Error('Save failed');
                        }
                      } catch {
                        toast({ title: isItalian ? 'Errore' : 'Error', description: isItalian ? 'Impossibile salvare.' : 'Could not save.', variant: 'destructive' });
                      }
                    }}
                    className="bg-foreground text-background hover:bg-foreground/90 h-10 px-5 rounded-lg text-sm font-medium"
                  >
                    <Sparkles className="h-4 w-4 mr-2" />
                    {isItalian ? 'Salva nella libreria' : 'Save to library'}
                  </Button>
                  <Button
                    variant="outline"
                  ...
                  >
                    {isItalian ? 'Genera un altro' : 'Generate another'}
                  </Button>
                </div>
              </div>
            )}
