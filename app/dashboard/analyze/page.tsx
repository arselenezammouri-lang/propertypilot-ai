...
export default function AnalyzePage() {
  ...
  const t = useMemo(
    () => getTranslation(locale as SupportedLocale).dashboard.analyzeLinkPage,
    [locale]
  );

  const pasteUrlRef = useRef(t.pasteUrl);
  pasteUrlRef.current = t.pasteUrl;
  const analysisErrorRef = useRef(t.analysisError);
  analysisErrorRef.current = t.analysisError;
  const copyFailedRef = useRef(t.copyFailed);
  copyFailedRef.current = t.copyFailed;

  const handleAnalyze = async () => {
    if (!url.trim()) {
      const v = validationToast(feedbackLocale, "linkAnalysis", pasteUrlRef.current);
  ...
  const copyToClipboard = async (text: string, label: string) => {
    try {
      ...
      const c = clipboardFailureToast(feedbackLocale, "linkAnalysis", copyFailedRef.current);
              ...
              <div className="futuristic-card p-6 border-neon-aqua/30">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      copyToClipboard(analysis.rewrittenListing.professional, t.copyProfessionalSnippetLabel)
                    }
              ...
              <div className="futuristic-card p-6 border-electric-blue/30">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => copyToClipboard(analysis.rewrittenListing.short, t.copyShortSnippetLabel)}
                ...
                <p className="text-sm leading-relaxed">{analysis.rewrittenListing.short}</p>
              </div>
                  ...
                  {analysis.rewrittenListing.titles.map((title, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-royal-purple/10 rounded-lg border border-royal-purple/20">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          copyToClipboard(title, t.copyTitleSnippetLabel.replace("{n}", String(i + 1)))
                        }
