...
export default function TitlesPage() {
  ...
  const TitleCard = ({ title, index, category }: { title: string; index: number; category: string }) => (
    <div 
      className="group flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-muted/50 hover:bg-slate-100 dark:hover:bg-muted transition-all border border-slate-200 dark:border-slate-700"
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            {generateMutation.isPending && (
              <Card className="border-0 shadow-xl shadow-indigo-500/10 bg-white dark:bg-card">
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <Button
                        ...
                        className="flex-shrink-0 bg-green-500 hover:bg-green-600 text-foreground"
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {TITLE_CATEGORIES.map((category) => {
                    ...
                    return (
                      <Card 
                        key={category.key}
                        className={`border-0 shadow-xl ${category.shadowColor} bg-white dark:bg-card overflow-hidden`}
                        data-testid={`card-${category.key}`}
                      >
                        <CardHeader className={`bg-gradient-to-r ${category.gradient} text-foreground py-4`}>
