...
export default function PipelinePage() {
  ...
  const statusConfig: Record<StatusColumn, { label: string; color: string; bgColor: string; borderColor: string }> = {
    new: { label: t.statusNew, color: 'text-blue-400', bgColor: 'bg-blue-500/10', borderColor: 'border-border' },
      ...
      <div className="max-w-[1800px] mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            <Link href="/dashboard">
              <Button variant="ghost" size="icon" className="text-slate-400 hover:text-foreground" data-testid="button-back-dashboard" aria-label="Back to dashboard">
          ...
          <div className="flex items-center gap-3">
            <Link href="/dashboard/leads">
              <Button variant="outline" className="border-slate-700 text-slate-300 hover:bg-muted" data-testid="button-table-view">
        ...
        <div className="mb-6 p-4 rounded-xl bg-muted/50 border border-slate-700/50">
            ...
            <div
              ...
              className={`flex flex-col rounded-xl border-2 transition-all duration-200 min-h-[500px] ${
                dragOverColumn === status
                  ...
                  : 'border-slate-700/50 bg-muted/30'
              }`}
                ...
                {getLeadsByStatus(status).length === 0 ? (
                  ...
                  getLeadsByStatus(status).map(lead => (
                    <div
                      ...
                      className={`group p-4 rounded-lg bg-card/80 border border-slate-700/50 cursor-grab active:cursor-grabbing hover:border-slate-600 transition-all ${
                    ...
                    >
                      <div className="flex items-start justify-between gap-2 mb-3">
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <span className="font-semibold text-foreground truncate">{lead.nome}</span>
                        </div>
                      ...
                      <div className="flex items-center gap-2 pt-2 border-t border-slate-700/50">
                        <Link href={`/dashboard/leads?selected=${lead.id}`} className="flex-1" aria-label={`Open lead ${lead.id}`}>
                          <Button 
                            variant="ghost" 
                            size="sm" 
                            className="w-full text-xs text-slate-400 hover:text-foreground hover:bg-muted"
        ...
        <div className="mt-8 p-4 rounded-xl bg-muted/30 border border-slate-700/50">
