        ...
        <div className="rounded-xl border border-border bg-muted/30 overflow-hidden">
          ...
          <div className="grid grid-cols-5 gap-4 p-4 border-b border-border">
            {Array.from({ length: 5 }).map((_, i) => (
              <Skeleton key={i} className="h-4 bg-muted/60" />
            ))}
          </div>
          {/* Data rows */}
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="grid grid-cols-5 gap-4 p-4 border-b border-border/50">
              ...
              <Skeleton className="h-4 w-20 my-auto bg-muted/50" />
              <Skeleton className="h-6 w-16 rounded-full my-auto bg-muted/50" />
              <Skeleton className="h-6 w-12 rounded-full my-auto bg-muted/50" />
              <Skeleton className="h-4 w-24 my-auto bg-muted/50" />
            </div>
          ))}
        </div>
      </div>
