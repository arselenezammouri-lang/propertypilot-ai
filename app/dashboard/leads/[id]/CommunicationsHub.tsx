  ...
  return (
    <Card className="bg-gradient-to-r from-indigo-900/30 to-violet-900/30 border-indigo-500/20">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {Object.entries(channelConfig).map(([channel, config]) => {
            ...
            return (
              <div 
                key={channel}
                className={`bg-muted/40 p-3 rounded-lg text-center cursor-pointer transition-all ${
                  activeChannel === channel ? 'ring-2 ring-indigo-500' : 'hover:bg-card/70'
                }`}
                onClick={() => setActiveChannel(channel as CommunicationChannel)}
                data-testid={`channel-${channel}`}
              >
                <Icon className={`h-5 w-5 mx-auto mb-1 ${config.color}`} />
                <p className="text-foreground font-semibold">{stats[channel as keyof typeof stats]}</p>
        ...
        {/* Compose Section */}
        <div className="space-y-4 bg-card/30 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Select value={tone} onValueChange={(v) => setTone(v as CommunicationTone)}>
                <SelectTrigger className="w-32 h-8 bg-muted border-slate-700 text-foreground text-xs" data-testid="select-tone">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {toneOptions.map(t => (
              ...
              <Select value={purpose} onValueChange={setPurpose}>
                <SelectTrigger className="w-32 h-8 bg-muted border-slate-700 text-foreground text-xs" data-testid="select-purpose">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          {activeChannel === 'email' && (
            <Input
              ...
              className="bg-muted border-slate-700 text-foreground placeholder:text-slate-500"
          ...
          <div className="relative">
            <Textarea
              ...
              className="bg-muted border-slate-700 text-foreground placeholder:text-slate-500 min-h-[120px] resize-none"
          ...
          {generatedVariants.length > 0 && (
            <div className="space-y-2">
              ...
              {generatedVariants.map((variant, i) => (
                <div key={i} className="bg-muted/50 p-2 rounded text-slate-300 text-sm flex items-start justify-between gap-2">
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {isLoadingLogs ? (
              ...
              logs.map((log) => {
                ...
                const Icon = config.icon;
                return (
                  <div key={log.id} className="bg-muted/40 p-3 rounded-lg flex items-start gap-3">
                    <div className={`p-2 rounded-full bg-muted`}>
                      <Icon className={`h-4 w-4 ${config.color}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      {log.subject && (
                        <p className="text-foreground text-sm font-medium truncate">{log.subject}</p>
                      )}
