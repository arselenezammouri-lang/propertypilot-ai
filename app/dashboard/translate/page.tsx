      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-5 gap-8">
            ...
            <Card className="border-slate-200/50 dark:border-slate-800/50 shadow-lg" data-testid="card-form">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger className="bg-white dark:bg-card" data-testid="select-tipo-transazione">
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl border-2 transition-all duration-200 flex flex-col items-center gap-1 ${
                        selectedLanguage === lang.code
                          ...
                          : 'border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-700 bg-white dark:bg-card'
                      }`}
                    ...
                    <button
                      ...
                      className={`p-4 rounded-xl border-2 transition-all duration-200 ${
                        selectedTone === tone.value
                          ? `border-transparent bg-gradient-to-br ${tone.gradient} text-foreground shadow-lg`
                          : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 bg-white dark:bg-card'
                      }`}
                      data-testid={`button-tone-${tone.value}`}
                    >
                      <div className="font-semibold">{tone.label}</div>
                      <div className={`text-xs mt-1 ${selectedTone === tone.value ? 'text-foreground/80' : 'text-slate-500'}`}>
                        {tone.description}
                      </div>
                    </button>
                  ))}
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            {!isLoading && !result && (
              <Card className="border-dashed border-2 border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-muted/40">
                  ...
                  <div className="flex flex-col items-center gap-4 text-center">
                    ...
                    <div className="flex flex-wrap justify-center gap-2 mt-4">
                      {LANGUAGES.slice(0, 6).map((lang) => (
                        <span key={lang.code} className="px-3 py-1 rounded-full bg-white dark:bg-muted text-sm border border-slate-200 dark:border-slate-700">
                          {lang.flag} {lang.name}
                        </span>
                      ))}
