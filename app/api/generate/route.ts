...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fire-and-forget: send upgrade nudge email at 50% usage
    const newUsage = currentUsage + 1;
    if (monthlyLimit > 0 && newUsage === Math.floor(monthlyLimit * 0.5)) {
      import('@/lib/resend-client').then(async ({ isEmailConfigured, getResendClient }) => {
        if (!isEmailConfigured()) return;
        try {
          const { client, fromEmail } = await getResendClient();
          const { emailTemplates } = await import('@/lib/email-templates');
          const userName = user.user_metadata?.full_name || user.email?.split('@')[0] || 'there';
          const template = emailTemplates.upgradeNudge(userName, newUsage, monthlyLimit);
          await client.emails.send({ from: fromEmail, to: user.email!, subject: template.subject, html: template.html });
        } catch {}
      }).catch(() => {});
    }
