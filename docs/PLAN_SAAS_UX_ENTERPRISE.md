...
**Stato D1 (parziale):** home `/dashboard` — titolo, sottotitolo e CTA da dizionario; widget da `getTranslation`: **GlobalLiveFeed**, **DashboardProTips**, **DashboardHelpButton**, **Dashboard3DStats**, **SniperStats**, **RegionalPortals**, **ProfitDashboard**, **MorningBriefingBox**, **CompetitorRadar**, **DashboardPlanCardsSection** (`planCards` + prezzi da `STRIPE_*` + `formatCurrencyForLocale`), **DashboardPlanFeatures** (`dashboard.planFeatures` + prezzi da `STRIPE_PLANS` + `billing.perMonth`); pagine **Opportunity Radar** (`/dashboard/opportunities`) e **Autopilot** (`/dashboard/autopilot`) — `dashboard.opportunitiesPage`, `dashboard.autopilotPage`; **Workspace settings** (`/dashboard/settings/workspace`) — `dashboard.workspacePage`; **Billing** — errore fetch soft `billing.subscriptionFetchErrorBody` / `billing.retry`; badge piano attivo con `billing.planBadgeStarter|Pro|Agency` + icone (no emoji). **Lead Manager** (`/dashboard/leads`) — `dashboard.leadsPage` (`lib/i18n/leads-page-ui.ts`) con codici mercato, badge score e priorità senza emoji; **dettaglio lead** — `dashboard.leadDetailPage` + `dashboard.communicationsHub` (tier score con icone); **pipeline Kanban** — `dashboard.leadPipelinePage` (legenda tier con icone); **CRM API keys** (`/dashboard/crm/settings`) — `dashboard.crmApiKeysPage`; **regole automazione CRM** (`/dashboard/crm/automations`) — `dashboard.crmAutomationRulesPage` (`lib/i18n/crm-automation-rules-page-ui.ts`); **workflow automazioni** (`/dashboard/automations`) — `dashboard.workflowAutomationsPage` (`lib/i18n/workflow-automations-page-ui.ts`); **branding agenzia** (`/dashboard/agency-branding`) — `dashboard.agencyBrandingPage` (`lib/i18n/agency-branding-page-ui.ts`); **notifiche Morning Intel** (`/dashboard/settings/notifications`) — `dashboard.notificationsSettingsPage` (`lib/i18n/notifications-settings-page-ui.ts`); **Agency Assistant** (`/dashboard/agency-assistant`) — `dashboard.agencyAssistantPage` (`lib/i18n/agency-assistant-page-ui.ts`); **Perfect Copy** (`/dashboard/perfect-copy`) — `dashboard.perfectCopyPage` (`lib/i18n/perfect-copy-page-ui.ts`); **Lead scoring** (`/dashboard/lead-score`) — `dashboard.leadScorePage` (`lib/i18n/lead-score-page-ui.ts`, mercati senza bandiere, badge categoria/template senza emoji); **Hashtag / Titoli A/B / Post social** — `dashboard.transactionTypes` (`lib/i18n/transaction-type-ui.ts`, select con icone Lucide); **Referral** (`/dashboard/referral`) — `dashboard.referralPage` (`lib/i18n/referral-page-ui.ts`); **Pacchetti premium** (`/dashboard/packages`) — `dashboard.packagesPage` (`lib/i18n/packages-page-ui.ts`); **Prospecting / Arbitrage Command Center** (`/dashboard/prospecting`) — `dashboard.prospectingPage` (`lib/i18n/prospecting-page-ui.ts`, `nextAction` IT/EN nel bundle + **ES/FR/DE/PT/AR** in `prospecting-next-action-ui.ts` applicati con `prospectingPageUiWithNextAction` nei partial `translations.*.dashboard`); `suggestNextAction(copy, context)`; date/prezzi `formatDateTimeForLocale` / `formatCurrencyForLocale`; portali `prospecting-platforms.ts`; WhatsApp `prospecting-whatsapp-outreach.ts`; tipi `lib/ai/next-action-types.ts`. **Analisi da link** (`/dashboard/analyze`) — `dashboard.analyzeLinkPage` (`lib/i18n/analyze-link-page-ui.ts`); **Scraper annunci** (`/dashboard/scraper`) — `dashboard.listingScraperPage` (`lib/i18n/listing-scraper-page-ui.ts`); **Audit annuncio** (`/dashboard/auditor`) — `dashboard.listingAuditorPage` (`lib/i18n/listing-auditor-page-ui.ts`); **Header dashboard** (`components/dashboard-header.tsx`) — `dashboard.navAriaLabel`, `dashboard.commandPaletteOpenAria`, `dashboard.commandPalettePlaceholder`, `generate`, `signOut`; **Nav dashboard** (sidebar, mobile drawer, command palette) — `dashboardNav` + `commandPaletteExtras` (`lib/i18n/dashboard-nav-ui.ts`, `lib/dashboard/nav-config.tsx`); **Error boundary / API error handler** — `errorBoundaryModule` (`lib/i18n/error-boundary-ui.ts`); **Welcome tour** — `welcomeTour`; **Onboarding wizard** — `onboardingWizard`; **Banner checkout in sospeso** — `pendingCheckoutBanner` (`lib/i18n/pending-checkout-ui.ts`); **Modal limite (Aria)** — `ariaLimitModal` (`lib/i18n/aria-limit-modal-ui.ts`); badge **Predator live** — `dashboard.predatorLiveBadge`. Pagina **Predator Map** (`/dashboard/map`) — `dashboard.mapPage` (legenda KPI, paywall, sidebar dettaglio). Pagine marketing/legal **About, Blog, Features, Privacy, Terms, Refund** — `marketingAbout`, `marketingBlog`, `marketingFeatures`, `privacyPolicyPage`, `termsPolicyPage`, `refundPolicyPage` (moduli in `lib/i18n/*-page-ui.ts`); **Blog post** (`/blog/[slug]`) — `blogPostPage`; **Pricing** — `landing.pricingPage` + `pricingPagePlans` (`lib/i18n/pricing-page-plans-ui.ts`); **Contatti** (`/contatti`) — `contact` (header/footer/subject demo, counter messaggio, submitting); **Demo** (`/demo`) — estensioni `demo` (testimonial, trust stats, CTA finale, WhatsApp prefill, iframe title, copyright); **Home marketing** (`/`) — tabella prezzi: header colonne da `landing.pricing.plans`; celle `multiUser` / `voiceCalling` in `landing.pricing.tableCells`; blocco Agency Boost in `landing.pricing.agencyBoost` (anche `productName`; IT/EN/ES/FR/DE/AR/PT); hero: loghi portali `landing.hero.trustedPortalLogos`, numeri KPI `landing.hero.statsValues`; sezione feature: CTA da `landing.features.*.cta`; callout benefit: icona `Lightbulb` (non emoji) + solo copy da dizionario; motore di ricerca: `landing.searchEngine.stepLabel` + `prospectingCycleCta` + `step2.demoScoreLabel`; aria-label hero da `landing.hero.signupAriaLabel` / `demoDashboardAriaLabel`; **Diamond / marketing header** — tagline e «Torna home» da dizionario. Stesso pattern benefit su `/platform` dove duplicato. **Docs** `/docs`: quick cards con icone al posto di emoji nei titoli. **Perfect Copy** empty badges: icone. **Translate** note culturali: lista senza emoji. **Aria Coach**: suggerimento upgrade con `upgradeTipPrefix` localizzato (no 💡 nel contenuto). Guida scalabilità: [I18N_SCALING.md](./I18N_SCALING.md). IT/EN in repo; altre lingue → merge su EN. Etichette `DeferredMount` dal cookie `propertypilot_locale`.
