...
**Stato D1 (parziale):** home `/dashboard` — titolo, sottotitolo e CTA da dizionario; **GlobalLiveFeed**, **DashboardProTips**, **DashboardHelpButton**, **Dashboard3DStats** (`stats3d`), **SniperStats** (`sniperStats`), **RegionalPortals** (`regionalPortals`) da `getTranslation`. IT/EN in repo; altre lingue → merge su EN. Etichette `DeferredMount` dal cookie `propertypilot_locale`.
