...
**Stato C2:** help contestuale — `ContextualHelpTrigger` nel `DashboardPageHeader`; `FieldHelpLabel`; `TooltipProvider` in `app/layout.tsx`; articoli in `doc-content.ts` + `resolveDocArticle`. Slug in `CONTEXTUAL_HELP_DOC_SLUGS` (`lib/docs/doc-static-links.ts`, parity test). Copertura estesa: **home** (`getting-started/welcome`), **libreria annunci** (`first-listing`), **tool copy** (hashtag, social, titoli, traduci, refine, emotional, audit → `perfect-copy`), **Lead manager / dettaglio / score / CRM API & automazioni / workflow automations** (`crm/pipeline`), **billing-adjacent** (pacchetti → `billing-guide`), **PDF** + **Agency Assistant** (`smart-briefing/client-ready`), **notifiche** + **Agent BIO** (`smart-briefing/briefing-guide`), **referral** + **branding** (`commercial/business-features`), oltre alle pagine già elencate (Perfect Copy, Pipeline, Billing, Workspace, Prospecting/Autopilot, Scraper/Analyze, Map, Opportunities, Follow-up/Video).
