...
**Stato D1 (parziale):** home `/dashboard` — titolo, sottotitolo e CTA da dizionario; widget da `getTranslation`: **GlobalLiveFeed**, **DashboardProTips**, **DashboardHelpButton**, **Dashboard3DStats**, **SniperStats**, **RegionalPortals**, **ProfitDashboard** (`profitDashboard`), **MorningBriefingBox** (`morningBriefing`). IT/EN in repo; altre lingue → merge su EN. Etichette `DeferredMount` dal cookie `propertypilot_locale`.
