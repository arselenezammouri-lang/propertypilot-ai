...
**Stato C2:** help contestuale — `ContextualHelpTrigger` (`components/contextual-help-trigger.tsx`) nel `DashboardPageHeader` (prop `contextualHelp`); `FieldHelpLabel` per tooltip su campi complessi; `TooltipProvider` in `app/layout.tsx`; articoli in `lib/docs/doc-content.ts` + `resolveDocArticle` in `lib/docs/doc-article.ts`; hub `/docs` aggiornato. Slug ammessi tipizzati in `CONTEXTUAL_HELP_DOC_SLUGS` (`lib/docs/doc-static-links.ts`, verificati da `docs-hub-doc-articles-parity.test.ts`). Pagine con `?` → docs: **Perfect Copy**, **Pipeline lead**, **Billing**, **Workspace settings** (`getting-started/workspace-setup`), **Prospecting** + **Autopilot** (`prospecting/arbitrage`), **Scraper** + **Analisi da link** (`prospecting/scraper-guide`), **Predator Map** (`territory/map-usage`), **Opportunity Radar** (`competitor/radar-guide`), **Follow-up Email** + **Video Script** (`ai-voice/call-scripts`).
