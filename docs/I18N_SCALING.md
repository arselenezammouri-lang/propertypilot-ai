...
1. **Single source of truth per le stringhe UI**  
   ...
   - **Global Live Feed** (`components/global-live-feed.tsx`): intestazione da `dashboard.liveNetwork` (cardTitle, liveBadge); righe attività con icone Lucide (no emoji); messaggi da `dashboard.liveFeed` come prima.
