...
6. **Naming delle chiavi**  
   ...
   - **Marketing pubblico** — `/about`, `/blog`, `/features`, `/blog/[slug]` (placeholder), `/pricing` (card piani + footer link): **IT/EN** nei moduli `marketing-pages-ui`, `blog-post-page-ui`, `pricing-page-plans-ui`; **ES/FR/DE/PT/AR** in `lib/i18n/marketing-public-locales.ts` (`marketingAbout*`, `marketingBlog*`, `marketingFeatures*`, `blogPostPage*`, `pricingPagePlans*`). **Privacy / Termini / Rimborsi** — **IT/EN** in `privacy-policy-page-ui.ts`, `terms-policy-page-ui.ts`, `refund-policy-page-ui.ts`; **ES/FR/DE/PT/AR** in `legal-policy-locales-*.ts` re-esportati da `legal-policy-locales.ts` (sezione 10 privacy: nota generica sull’autorità di controllo, senza blocco Garante IT; revisione legale esterna consigliata prima del go-live).  
