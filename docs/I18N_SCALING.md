...
1. **Single source of truth per le stringhe UI**  
   ...
   - Prospecting / Arbitrage Command Center (`/dashboard/prospecting`): `lib/i18n/prospecting-page-ui.ts` → `dashboard.prospectingPage` (include `nextAction` per badge “Prossima azione”); helper `lib/i18n/prospecting-next-action-ui.ts` + logica `lib/ai/next-action-suggestion.ts` + tipi `lib/ai/next-action-types.ts`; portali: `lib/i18n/prospecting-platforms.ts`; WhatsApp proprietario: `lib/i18n/prospecting-whatsapp-outreach.ts`.  
