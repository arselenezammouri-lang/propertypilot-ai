...
6. **Naming delle chiavi**  
   ...
   - **Marketing pubblico** — `/about`, `/blog`, `/features`, `/blog/[slug]` (placeholder), `/pricing` (card piani + footer link): **IT/EN** nei moduli `marketing-pages-ui`, `blog-post-page-ui`, `pricing-page-plans-ui`; **ES/FR/DE/PT/AR** in `lib/i18n/marketing-public-locales.ts` (`marketingAbout*`, `marketingBlog*`, `marketingFeatures*`, `blogPostPage*`, `pricingPagePlans*`). **Privacy / Termini / Rimborsi** restano IT/EN nei file `*-policy-page-ui.ts` (traduzione legale completa = passo successivo).  
