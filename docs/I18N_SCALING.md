...
1. **Single source of truth per le stringhe UI**  
   ...
   - Home marketing (`app/(marketing)/page.tsx`): celle tabella prezzi localizzate in `landing.pricing.tableCells` (`multiUserAgency`, `voiceCallingPro`); blocco **Agency Boost** in `landing.pricing.agencyBoost` (badge, tier, bullet, CTA, pagamento una tantum); hero `signupAriaLabel` / `demoDashboardAriaLabel`; card feature `landing.features.*.cta`; motore di ricerca `landing.searchEngine.stepLabel` (`{n}`) e `prospectingCycleCta`.  
