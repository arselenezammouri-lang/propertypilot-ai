...
1. **Single source of truth per le stringhe UI**  
   ...
   - Lead Manager (`/dashboard/leads`): `lib/i18n/leads-page-ui.ts` → `dashboard.leadsPage` (placeholder `{name}` dove serve); anche `marketItalyCode` / `marketUsaCode`, `badgeRevenueOpportunity`, `badgeTopDeal`; in UI priorità con icone Lucide, mercato con codice + label.  
   ...
   - Prospecting / Arbitrage Command Center (`/dashboard/prospecting`): `lib/i18n/prospecting-page-ui.ts` → `dashboard.prospectingPage` (include `nextAction` per badge “Prossima azione”); `nextAction` è tradotto **IT/EN** nel modulo + **ES/FR/DE/PT/AR** in `prospecting-next-action-ui.ts`, applicato via `prospectingPageUiWithNextAction` negli oggetti parziali `translations.es|fr|de|pt|ar.dashboard` (merge profondo col resto EN); logica `lib/ai/next-action-suggestion.ts`; tipi `lib/ai/next-action-types.ts`; portali: `lib/i18n/prospecting-platforms.ts`; WhatsApp proprietario: `lib/i18n/prospecting-whatsapp-outreach.ts`.  
